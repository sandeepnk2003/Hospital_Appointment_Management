    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                <a href="/users/delete/<?= $user['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete user?')">Delete</a>
          <?php if($user['role'] == 'doctor' && !in_array($user['id'], $doctorUserIds)): ?>
        <a href="<?= base_url('users/add_from_user/'.$user['id']) ?>" class="btn btn-danger btn-sm">Add Doctor Info</a>
    <?php endif; ?>
            </td>
        </tr>
        <?php endforeach; ?>
